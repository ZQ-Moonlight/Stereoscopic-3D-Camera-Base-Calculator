...
- **三区安全边界切割**：2D 俯视图中引入精准的安全区色块遮罩。出屏超限区（红）、安全景深区（绿）、入屏超限区（灰），结合动态物理网格（如 1m, 5m 等比缩放），让景深厚度与视差违例情况一目了然。

### Changed
- **视差条物理直觉化重构**：将视差预算条（Budget Bar）的 UI 布局进行了镜像反转。现在左侧代表“出屏（负视差，靠近相机）”，右侧代表“入屏（正视差，退入背景）”，彻底对齐摄影师的真实空间直觉。
- **2D 图表渲染逻辑与语义深度优化**：
  - 明确横向深度轴为“真实比例（to scale）”，纵向相机间距为“示意放大”，并在图内给出明确标注。
  - 引入了三层标签防重叠智能布局，彻底解决 ZPS、Limit 与 Act N/L 距离过近时的文字遮挡问题。
  - 增加“超限 (Over)”动态状态反馈：当实际景物越过安全边界时，实体条和文本会瞬间变为警告色并追加超限标识。
  - 完全重构 SVG 的色彩引擎以兼容 CSS 变量，并实现了全图表无死角的 EN/ZH 双语无缝切换。

## [3.4.0] - 2026-04-03
### Added
- **“后期出屏预留空间”推演引擎**：针对 3D 电影工业中“幕内安全拍摄，后期释放出屏”的常见流程，新增专项推演。当使用无出屏构图（N=S）时，系统会自动利用剩余的正视差预算，为你精准算出：后期剪辑时画面向外平移的最高安全像素量（px），以及可换取的出屏毫米数（mm）。
- **悬浮气泡提示系统 (Floating Tip Bubble)**：废弃了占用屏幕上方空间的横幅提示，重构为小巧的右下角悬浮气泡。

### Changed
- 优化悬浮气泡的交互逻辑，加入“呼吸灯红点”提醒机制：当调整参数触发了新的诊断分析，且气泡处于收起状态时，红点才会闪烁，既智能提醒又极大地降低了视觉打扰。

### Fixed
- 修复当主体即为最近景（N=S 绝对安全构图）时，被底层逻辑错误拦截并报出“N 必须小于 S”的 Bug。
